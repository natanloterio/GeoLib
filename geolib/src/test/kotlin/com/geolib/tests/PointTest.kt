...
class PointTest {
    ...
    @Test
    fun `Throws exception if latitude greater than 180`() {
        val longitude = 80.0000001 // should be between -90.0 and 90.0
        val latitude = 180.04956798875554

        try {
            Point.create(longitude = longitude, latitude = latitude)
            junit.framework.Assert.fail("Should've thrown an exception when latitude greater than 180")
        } catch (e: InvalidParameterException) {
            assertEquals(e.message,"Latitude must be within -180.0 and 180.0. But it was %s".format(latitude))
        }
    }

    @Test
    fun `Throws exception if latitude lower than -180`() {
        val longitude = 80.0000001 // should be between -90.0 and 90.0
        val latitude = -180.04956798875554

        try {
            Point.create(longitude = longitude, latitude = latitude)
            junit.framework.Assert.fail("Should've thrown an exception when latitude lower than -180")
        } catch (e: InvalidParameterException) {
            assertEquals(e.message,"Latitude must be within -180.0 and 180.0. But it was %s".format(latitude))
        }
    }
